...
public class IntegrationEvent {
	...
	public static void main(String[] args) {
		...
		try (BufferedWriter writer = new BufferedWriter(new FileWriter(OUTPUT_FILE_PATH))) {
			...
			List<Event> qnlEventList = new ArrayList<>(
					integration.loadEventsFromJson(objectMapper, QNL_EVENTS_JSON_PATH));
			...
			while (qnlIterator.hasNext()) {
				...
				for (int i = 0; i < wikiEventList.size(); i++) {
					...
					if (integration.areEquals(wikiStartDate, wikiEndDate, qnlStartDate, qnlEndDate)) {
						writer.write("Events " + integration.formatEventDetails(wikiEventList.get(i)) + " and "
								+ eventQNL.getEvent() + " are the same.\n");
						foundMatch = true;
						count++;

						if (integration.compareDates(wikiStartDate, qnlStartDate) == 0) {
							if (wikiEndDate != null && qnlEndDate != null
									&& integration.compareDates(wikiEndDate, qnlEndDate) <= 0) {
								...
								writer.write(
										"New Event: " + integration.formatEventDetails(wikiEventList.get(i)) + "\n");
								break;
							}
						...
						if (integration.compareDates(wikiStartDate, qnlStartDate) < 0) {
							Event event = wikiEventList.get(i);
							if (wikiEndDate != null && qnlEndDate != null
									&& integration.compareDates(wikiEndDate, qnlStartDate) == 0) {
								event.setTo(eventQNL.getTo());
							}
							event.setEvent(event.getEvent() + ". " + eventQNL.getEvent());
							wikiEventList.set(i, event);
	...
	private boolean areEquals(List<Integer> wikiStartDate, List<Integer> wikiEndDate, List<Integer> qnlStartDate,
			List<Integer> qnlEndDate) {
