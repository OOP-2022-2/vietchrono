		...
		try {
			...
			rawDateList.forEach(str -> {
				...
				Pattern pattern = Pattern.compile(
						"\\b(\\d{1,2}\\.\\d{1,2}\\.\\d{1,4}( TCN| SCN)?|\\d{1,2}-\\d{1,2}-\\d{1,4}( TCN| SCN)?|\\d{1,2}\\.\\d{1,4}( TCN| SCN)?|\\d{1,4}( TCN| SCN)?)\\b");
				Matcher matcher = pattern.matcher(str);

				List<String> matchedDates = new ArrayList<>();
				while (matcher.find()) {
					String date = matcher.group();
					if (!matchedDates.contains(date))
			...
			for (String str : rawDateList) {
				...
				if (startDate.equals("113"))
					startDate = startDate + " TCN";
				Event event = new Event(strList.get(index), HelperDateConverter.convertDate(startDate, "/"),
						HelperDateConverter.convertDate(endDate, "/"), url);
				events.add(event);
				index++;
			}
