...
public class CollectorEventWiki extends CollectorEvent {
	...
	private void processParagraph(Element element, String ageText, List<String> dynastyList, List<String> yearList,
		...
		String endYear = years.length > 1
				? (years[1].contains("tháng ") ? years[1].replace("tháng ", "").trim() : years[1].trim())
				: "";
		...
		if (!event.isBlank()) {
			...
			for (int i = 0; i < links.size(); i++) {
				...
				description = description + " "
						+ (eleDocument.select("p").size() != 0 ? eleDocument.select("p").get(0).text() : "");
				Elements imageUrls = doc.getElementsByClass("mw-file-description");
				for (int j = 0; j < imageUrls.size(); j++) {
					String imgUrl = imageUrls.get(j) == null || imageUrls.get(j).attr("href").contains(".svg|.SVG") ? ""
							: "https://vi.wikipedia.org" + imageUrls.get(j).attr("href");
					String caption = imageUrls.get(j).nextElementSibling() == null ? ""
							: imageUrls.get(j).nextElementSibling().text();
			...
			Event eventEntity = new Event(ageText, dynastyList.get(0), event,
					HelperDateConverter.convertDate(startYear, " "), HelperDateConverter.convertDate(endYear, " "),
	...
	private void processDefinitionList(Element element, String ageText, List<String> dynastyList, List<String> yearList,
		...
		ddElements.forEach(ddElement -> {
			...
			String startYear = (years[0].contains("tháng ") ? years[0].replace("tháng ", "") : years[0]).trim()
					.concat(" ").concat(yearList.get(0));
			String endYear = years.length > 1
					? (years[1].contains("tháng ") ? years[1].replace("tháng ", "") : years[1]).trim().concat(" ")
							.concat(yearList.get(0))
					: "";
