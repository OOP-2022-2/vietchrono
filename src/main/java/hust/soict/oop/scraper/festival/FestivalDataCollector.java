...
public class FestivalDataCollector {
	...
	private static Festival insertData(String name, String date, String description, String location, Image image,
			String source) {
	...
	public static void main(String[] args) {
		try {
					...
					if (i == 0) {
						for (int k = 0; k < listTd.size(); k++) {
							if (k == 0) {
								...
								if (listTd.get(k).firstElementChild() != null) {
									...
									if (href != "") {
										...
										for (int x = 0; x < imageSrc.size(); x++) {
											String imageUrl = (imageSrc.get(x) == null
													|| imageSrc.get(x).attr("href").contains(".svg|.SVG")) ? ""
															: imageSrc.get(x).attr("href");
											String caption = (imageSrc.get(x).nextElementSibling() == null) ? ""
													: imageSrc.get(x).nextElementSibling().text();
											if (imageUrl != "" && caption != "") {
								...
								date = timeReformatter(getCleanData(listTd.get(k).text().trim()));
							else if (k == 2) {
								String descField = listTd.get(k).text().trim();
								description = (descField != "") ? getCleanData(listTd.get(k).text().trim())
										: "Not Available";
							}
						}
						location = "Toàn quốc";
							...
							if (k == 0)
								...
								if (listTd.get(k).firstElementChild() != null) {
									...
									if (href1 != "") {
										...
										Elements listDesc = imgNDescDoc
												.select("#mw-content-text > div.mw-parser-output > p");
										Elements listDesc1 = imgNDescDoc
												.select("#mw-content-text > div.mw-parser-output > p.mw-empty-elt");
								...
								location = getCleanData(listTd.get(k).text().trim());
						}
					}
			...
			try (FileWriter fileWriter = new FileWriter(
					"src/main/java/hust/soict/oop/scraper/festival/data/festivals.json")) {
